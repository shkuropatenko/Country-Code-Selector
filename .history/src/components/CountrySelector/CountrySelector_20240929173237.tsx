...
type CountryCode = {
  callingCode: string;
  // add other properties if needed, like name, flag, etc.
};
...
const CountrySelector: React.FC = () => {
  ...
  const [autocompleteOpen, setAutocompleteOpen] = useState(false); // Состояние для управления открытием автокомплита
    ...
    <FormProvider {...methods}>
      <form onSubmit={methods.handleSubmit(onSubmit)}>
        <Box sx={{ mb: 2, display: "flex", alignItems: "center" }}>
          {/* Отображение текущей выбранной страны */}
          <Box
