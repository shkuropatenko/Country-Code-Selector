...
const CountrySelector: React.FC = () => {
  ...
  const handleCountrySelect = (event: any, country: any | null) => {
    ...
    // Remove the '+' from calling_code for comparison
    const cleanCallingCode = country.calling_code.replace("+", "");

    const code = countryCodes.find(
      (c) => c.callingCode === cleanCallingCode // Compare without the '+'
    );
          ...
          <Controller
            ...
            render={({ field }) => (
              <Autocomplete
                ...
                value={selectedCountry} // Controlled value prop
                onChange={handleCountrySelect} // Controlled onChange handler
                  ...
                  return (
                    ...
                    >
                      <Avatar
                        src={
                          code
                            ? `https://flagcdn.com/w20/${code.isoCode.toLowerCase()}.png`
                            : ""
                        }
                        alt={option.name}
                        sx={{ mr: 1, width: 20, height: 11, borderRadius: 0 }} // Задаем меньший размер
                      />
