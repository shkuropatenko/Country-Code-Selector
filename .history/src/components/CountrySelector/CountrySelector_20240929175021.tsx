...
const CountrySelector: React.FC = () => {
  ...
  const boxRef = useRef<HTMLDivElement>(null); // Ref to the box
  ...
  return (
    <FormProvider {...methods}>
      <form onSubmit={methods.handleSubmit(onSubmit)}>
        <Box sx={{ mb: 2, display: "flex", alignItems: "center" }} ref={boxRef}>
          <Box
            onClick={() => setAutocompleteOpen((prev) => !prev)} // Toggle autocomplete on click
            sx={{
              cursor: "pointer",
              display: "flex",
              alignItems: "center",
