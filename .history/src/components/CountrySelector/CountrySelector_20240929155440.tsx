            ...
            render={({ field }) => (
              <Autocomplete
                ...
                renderOption={(props, option) => {
                  const { key, ...restProps } = props; // Извлекаем ключ
                  return (
                    <Box
                      component="li"
                      key={key} // Передаем ключ напрямую
                      {...restProps}
                      sx={{ display: "flex", alignItems: "center" }}
                    >
                      <Avatar
                        src={`https://flagcdn.com/w20/${option.id.toLowerCase()}.png`} // Используем ISO-код страны для загрузки флага
                        alt={option.name}
                        sx={{ mr: 1 }}
                      />
