...
const CountrySelector: React.FC = () => {
  ...
  // Update mask based on selected country
  const handleCountryChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    const selectedCountry = countries.find(
      (country) => country.id === event.target.value
    );
    // Update the mask according to the selected country's phone number format
    setMask(selectedCountry?.phone_number_format || "(999) 999-9999"); // Assume `phone_number_format` contains the required mask
  };
