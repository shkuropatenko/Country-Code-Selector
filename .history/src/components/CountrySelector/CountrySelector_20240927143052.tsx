...
const CountrySelector: React.FC = () => {
  ...
  const handleCountryChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (selectedCountry) {
      const phoneLength = selectedCountry.phone_length; // Retrieve phone length
      if (phoneLength) {
        // Create mask and placeholder based on phone length
        const newMask = `(${"0".repeat(phoneLength[0])}) ${"0".repeat(
          phoneLength[1]
        )}-${"0".repeat(phoneLength[2])}`;
        const newPlaceholder = newMask.replace(/0/g, "0"); // Placeholder
        setMask(newMask); // Update mask
        setPlaceholder(newPlaceholder); // Update placeholder
      }
    } else {
