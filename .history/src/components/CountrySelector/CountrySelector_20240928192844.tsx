      ...
      <form onSubmit={methods.handleSubmit(onSubmit)}>
        <Box sx={{ mb: 2, display: "flex", alignItems: "center" }}>
          ...
          <Controller
            ...
            render={(
              { field }: { field: any } // или укажите более точный тип
            ) => (
              <InputMask
                mask={mask}
                value={field.value}
            ...
            <Controller
              ...
              render={({ field }) => (
                <InputMask
                  ...
                  value={field.value}
                  onChange={field.onChange}
                  // Убедитесь, что вы добавляете нужные пропсы
                  {...field}
                >
                  {(
                    inputProps // Передаем функцию в качестве дочернего элемента
                  ) => (
                    <TextField
                      {...inputProps} // Передаем все нужные свойства из inputProps
                      label="Phone Number"
                      variant="outlined"
                      placeholder={placeholder}
                      fullWidth
